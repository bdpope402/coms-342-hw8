...
public class Evaluator implements Visitor<Value, Env<Value>> {
	...
	public Value visit(AddExp e, Env<Value> env) {
		...
		if(operands.get(0) instanceof AST.ListExp) {
			List<Exp> elemExps = ((ListExp) operands.get(0)).elems();
			int length = elemExps.size();
			Value[] elems = new Value[length];
			for(int i=0; i<length; i++)
				elems[i] = (Value) elemExps.get(i).accept(this, env);

			Value result2 = new Value.Null();
			for(int i=length-1; i>=0; i--)
				result2 = new PairVal(elems[i], result2);
			return result2;
		}
	...
	@Override
	public Value visit(SwitchExp e, Env<Value> env) { // New for switch.
